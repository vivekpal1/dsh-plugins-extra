...
import { catalog, countPlugin, packageRoot, resolvePackages } from "./catalog.js";
...
export function parseArguments(argv) {
  const positionals = [];
  const options = { profile: process.env.DSH_PROFILE || "web", dryRun: false, json: false };
...
async function install(entries, options, dependencies) {
  ...
  for (const entry of entries) {
    io.out(`\n${entry.key} (${entry.category}): ${entry.disclosure}\n`);
    if (options.dryRun) {
      io.out(`Would install ${entry.packageName} into DSH profile '${options.profile}'.\n`);
      continue;
    }
    const archive = await packPackage(system, entry, archiveDirectory);
    io.out(`Installing ${entry.packageName} into DSH profile '${options.profile}'…\n`);
    system.run("dsh", ["plugin", "--profile", options.profile, "add", archive], { inherit: true });
    ...
    io.out(`Installed ${entry.packageName}.\n`);
  }
  if (!options.dryRun) io.out("\nRestart DSH to load the installed extensions.\n");
}
...
async function uninstall(entries, options, dependencies) {
  ...
  for (const entry of entries) {
    ...
    system.run("dsh", ["plugin", "--profile", options.profile, "remove", entry.packageName], { inherit: true });
    const count = countPlugin(dumpConfig(system, options.profile), entry.pluginId);
