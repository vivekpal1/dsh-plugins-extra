...
export const catalog = Object.freeze([
  ...
  Object.freeze({
    ...
    summary: "Control selected DSH sessions from a paired Telegram bot.",
...
export function resolvePackages(values) {
  ...
  if (requested.length === 0) throw new Error("Choose at least one package: codex, import, wallet, themes, terminal, or telegram");
  const expanded = requested.includes("all") ? catalog : requested.map((value) => {
    const entry = byAlias.get(value);
    if (!entry) throw new Error(`Unknown package '${value}'. Choose codex, import, wallet, themes, terminal, telegram, or all`);
    return entry;
  });
