...
const SAFE_ERROR = /^(?:A Solana wallet already exists|A Solana transfer is already in progress|Create or import a wallet first|Create or import a wallet in Settings first|Wallet password must contain at least 12 characters|Recovery phrase is required|Recovery phrase is not valid BIP39 English|Wallet password is incorrect or the encrypted wallet is damaged|Encrypted wallet does not match its public address|Unlock the wallet in Settings before signing|Recipient is not a valid Solana address|Recipient must be different from this wallet|Enter a positive SOL amount with no more than 9 decimal places|SOL amount must be greater than zero|SOL amount is outside the supported range|Wallet balance must cover the amount and transaction fee|Solana could not estimate the transaction fee|Solana transaction simulation failed|Unsupported Solana network|Mainnet selection requires explicit acknowledgement|Wallet network changed; review the transfer again|Solana RPC returned an unexpected transaction signature|Solana transaction failed during confirmation|Solana transaction expired before confirmation|Solana transaction confirmation timed out)$/u;
...
export function enabledNetwork(settings) {
  const configured = settings.get();
  if (configured.network === "mainnet-beta" && configured.mainnetEnabled === true) return "mainnet-beta";
  return "devnet";
}

export function createWalletRpcHandler({ vault, wallet, settings, getNetwork = () => enabledNetwork(settings) }) {
  return async (endpoint, payload, signal) => {
    try {
      signal.throwIfAborted();
      let value;
      if (endpoint === "status") {
        value = { ...await vault.status(), network: getNetwork(), autoLockMinutes: 5 };
        ...
        await settings.update({ network, mainnetEnabled: network === "mainnet-beta" });
        value = { network };
        ...
        value = await wallet.send(payload?.recipient, payload?.amountSol, {
          expectedNetwork: payload?.expectedNetwork,
          signal,
        });
      } else {
...
export function apply(ctx) {
  const settings = ctx.settings.register(settingsNamespace(SETTINGS_NAMESPACE), z.object({
    ...
    mainnetEnabled: z.boolean().default(false),
  }));
  const vault = new WalletVault(ctx.credentials, CREDENTIAL_REF);
  const getNetwork = () => enabledNetwork(settings);
  ...
  ctx.effect(
    () => ctx.connection.rpc.handle(CHANNEL, createWalletRpcHandler({ vault, wallet, settings, getNetwork }), { authority: "loopback" }),
