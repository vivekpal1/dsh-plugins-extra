...
New wallets start on Solana devnet. Mainnet must be selected explicitly and acknowledged in Settings. The selected network is persisted with that acknowledgement, and every reviewed transfer is bound to the same network before it can be signed. This release supports native SOL; SPL token support is planned separately.
